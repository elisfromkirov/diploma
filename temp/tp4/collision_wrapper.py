...
class CollisionWrapper:
    ...
    # geometry.collisionPairs <- array of all possible collision pairs
    # geometry_data.collisionResults <- array of details of collisions

    # computeCollisions() populates gdata.collisionResults list with computed values
    # computeDistances() populates gdata.distanceResults list with computed values

    # computeJointJacobians() populates rdata.J
    def computeCollisions(self,q,vq=None):
        ...
        if vq is not None:
            # updates (why does it need to be called)
            pin.forwardKinematics(self.rmodel,self.rdata,q,vq,0*vq)
        return res

    # return tuples consisting of collision pair index, collision pair and collision details
    # take them from geometry_model.collisionPairs
    ...
    def _getCollisionJacobian(self,col,res):
        ...
        # not just jacobian
        # it's matrix that get normal component of velocity in contant frame from q
        J = (Jc2-Jc1)[2,:]
